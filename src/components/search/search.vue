<script setup lang="ts">
  ...
  const key =  "caab8f3d7152469aa78113103232109"
    ...
    <div class="container">
      <form>
        ...
        <div class="relative">
          ...
          <input @input="getSearchResult" v-model="querySearch" type="search" class="block w-full p-4 pl-10 text-sm shadow text-gray-900 border rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Chercher une ville.." required>
        </div>
      </form>
      <div v-if="resultSearch" class="bg-white w-full rounded-lg shadow-xl overflow-hidden mt-3">
        <ul>
          ...
          <template v-else>
            <li v-for="result in resultSearch" :key="result.id" class="py-2 p-3 cursor-pointer hover:bg-violet-200 transition-colors" @click="viewCity(result)">
